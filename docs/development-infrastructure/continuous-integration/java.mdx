...
Also make sure that:
  ...
  3. your GitHub Action includes `configure-java`, using the following `server-*` and `gpg-*` parameters [shown here](https://github.com/finos/finos-parent-pom/blob/finos-7/.github/workflows/build.yml#L38)
...
### Known issues
If the release command fails with "gpg: signing failed: Inappropriate ioctl for device", run the following command and try again (see also the [GnuPG agent documentation](https://www.gnupg.org/documentation/manuals/gnupg/Invoking-GPG_002dAGENT.html)).
```
export GPG_TTY=$(tty)
```
