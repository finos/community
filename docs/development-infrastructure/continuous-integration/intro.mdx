...
Continuous Integration (CI) runs validation automatically at commit and merge time, helping teams detect problems earlier. CI setup is language- and ecosystem-specific (see `Release and deployment` below).

The Open Developer Platform helps FINOS projects adopt CI tools and services, including options provided by vendors that support the FINOS community, like WhiteSource, GitHub and RedHat.

The sections below cover CI engines used across FINOS projects. While many projects historically used Travis CI, most have moved toward GitHub Actions since [its release in 2019](https://github.blog/2019-08-08-github-actions-now-supports-ci-cd/).
...
Travis CI can easily publish packages to npm using [semantic-release](https://github.com/semantic-release), which delegates release operations to your CI environment and allows you to control versioning [using commits (commitizen)](https://www.npmjs.com/package/commitizen); as a result, npm packages will be continuously released on each code repository change.
