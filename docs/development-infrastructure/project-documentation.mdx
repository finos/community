...
FINOS project documentation is typically authored with Docusaurus and deployed with Netlify. This combination keeps docs easy to write in markdown, easy to review in pull requests, and easy to publish with preview environments.
...
2. Easy to collaborate - Files are stored in a `docs/` folder in the GitHub repository, allowing anyone to submit a Pull Request.
3. Automated - Changes to `main` trigger a website deployment.
...
Website pages are represented by Markdown files in the `docs/` folder (in Docusaurus 3, Markdown React files are also supported). The page `id` maps to the URL slug of the page; the title renders out as an `<h1>` on the page.
...
See the [Docusaurus docs](https://docusaurus.io/docs/docs-introduction) documentation page for more information.
...
The sidebar that is used to navigate the doc files in `docs/` folder is managed in the `website/sidebars.js` file. The sidebar links to each doc file via the `id` property set in the .md or .mdx file. 
...
The [Accessibility Theme Builder](https://a11y-theme-builder.finos.org/) project website utilizes [MKDocs](https://www.mkdocs.org/) (specifically [Material for MKDocs](https://squidfunk.github.io/mkdocs-material/getting-started/)).
...
Netlify is the preferred tool by FINOS when it comes to hosting websites, as it offers hosting and serverless backend services for web applications and static websites, along with seamless integration with GitHub and automatic preview capabilities that make it easier to review changes before they go live.
